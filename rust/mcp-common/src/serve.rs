...
pub async fn tls_config() -> Option<RustlsConfig> {
    let cert = std::env::var("ATLAS_TLS_CERT").ok().filter(|value| !value.is_empty())?;
    let key = std::env::var("ATLAS_TLS_KEY").ok().filter(|value| !value.is_empty())?;
    Some(RustlsConfig::from_pem_file(cert, key).await.expect("invalid TLS certificate or key"))
}
